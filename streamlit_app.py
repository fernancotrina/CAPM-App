...
market_ticker = tickers_indices[nombres_indices.index(indice_seleccionado_nombre)]
    ...
    # Elección de Tickers
tickers_input = st.sidebar.text_input("Tickers separados por espacio", "NVDA LLY GOOG JPM")
...
if st.sidebar.button("🚀 Ejecutar Análisis CAPM", type="primary"):
    ...
    stock_prices = prices.drop(columns=market_ticker)

    stock_returns = calculate_returns(prices, return_type)

    ## No importa ahora
    avg_mkt_ret = stock_returns[market_ticker].mean() * 252
    ...
    st.plotly_chart(plot_cumulative_returns(stock_returns), use_container_width=True)
    ...
    report = finaal_report(stock_returns, market_ticker, risk_free_rate)
    st.dataframe(report)

    st.subheader("📉 Betas Rolling")
    rolling_betas = rolling_beta(stock_returns, market_ticker, risk_free_rate, window)
